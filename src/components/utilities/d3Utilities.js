...
export function massPlanet(stateUI) {
	return function _massPlanet(d) {
		...
		return {
			...
			'animation-duration': `${parseFloat(orbitDuration / 4).toFixed(2)}s`,
			'background': `radial-gradient(circle at 50%, rgba(0,0,0,0.1) 17%, rgba(0,0,0,0.43) 91%), url(/planetDecision/static/media/${d.name.toLowerCase()}.jpg) 0 0/cover rgb(${d.colour})`,
			'font-size': `${calc/4}px`,
		};
	};
}
